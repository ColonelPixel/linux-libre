...
static int stlc45xx_reject_firmware(struct stlc45xx *stlc)
{
	const struct firmware *fw;
	int ret;

	/* FIXME: should driver use it's own struct device? */
	ret = reject_firmware(&fw, "/*(DEBLOBBED)*/", &stlc->spi->dev);

	if (ret < 0) {
		stlc45xx_error("reject_firmware() failed: %d", ret);
		return ret;
	}
...
static int stlc45xx_upload_firmware(struct stlc45xx *stlc)
{
	...
	if (!stlc->fw) {
		ret = stlc45xx_reject_firmware(stlc);
		if (ret < 0)
			return ret;
	}
