...
static int sep_load_firmware(struct sep_device *sep)
{
	...
	char *cache_name = "/*(DEBLOBBED)*/";
	char *res_name = "/*(DEBLOBBED)*/";
	char *extapp_name = "/*(DEBLOBBED)*/";
	int error ;
	...
	error = reject_firmware(&fw, res_name, &sep->pdev->dev);
	if (error) {
	...
	error = reject_firmware(&fw, cache_name, &sep->pdev->dev);
	if (error) {
	...
	error = reject_firmware(&fw, extapp_name, &sep->pdev->dev);
	if (error) {
