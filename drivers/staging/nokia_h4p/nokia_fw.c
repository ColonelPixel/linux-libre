...
#define FW_NAME_TI1271_PRELE	"/*(DEBLOBBED)*/"
#define FW_NAME_TI1271_LE	"/*(DEBLOBBED)*/"
#define FW_NAME_TI1271		"/*(DEBLOBBED)*/"
#define FW_NAME_BCM2048		"/*(DEBLOBBED)*/"
#define FW_NAME_CSR		"/*(DEBLOBBED)*/"
				 ...
				 const struct firmware **fw_entry)
{
	...
	case H4P_ID_TI1271:
		switch (info->ver_id) {
		case 0xe1:
			err = reject_firmware(fw_entry, FW_NAME_TI1271_PRELE,
						info->dev);
			break;
		case 0xd1:
		case 0xf1:
			err = reject_firmware(fw_entry, FW_NAME_TI1271_LE,
						info->dev);
			break;
		default:
			err = reject_firmware(fw_entry, FW_NAME_TI1271,
						info->dev);
		}
		break;
	case H4P_ID_CSR:
		err = reject_firmware(fw_entry, FW_NAME_CSR, info->dev);
		break;
	case H4P_ID_BCM2048:
		err = reject_firmware(fw_entry, FW_NAME_BCM2048, info->dev);
		break;
	default:
