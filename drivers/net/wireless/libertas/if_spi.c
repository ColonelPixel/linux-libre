...
static int if_spi_prog_helper_firmware(struct if_spi_card *card)
{
	...
	err = reject_firmware(&firmware, card->helper_fw_name, &spi->dev);
	if (err) {
		lbs_pr_err("reject_firmware failed with err = %d\n", err);
		goto out;
	}
...
static int if_spi_prog_main_firmware(struct if_spi_card *card)
{
	...
	/* Get firmware image */
	err = reject_firmware(&firmware, card->main_fw_name, &spi->dev);
	if (err) {
			      ...
			      char *helper_fw, char *main_fw)
{
	...
	snprintf(helper_fw, IF_SPI_FW_NAME_MAX, "/*(DEBLOBBED)*/",
		 chip_id_to_device_name[i].name);
	snprintf(main_fw, IF_SPI_FW_NAME_MAX, "/*(DEBLOBBED)*/",
		 chip_id_to_device_name[i].name);
	return 0;
}
