...
static const char primary_fw_name[] = "/*(DEBLOBBED)*/";
static const char secondary_fw_name[] = "/*(DEBLOBBED)*/";
...
spectrum_dl_firmware(hermes_t *hw, struct pcmcia_device *link)
{
	...
	if (reject_firmware(&fw_entry, primary_fw_name,
	...
	if (reject_firmware(&fw_entry, secondary_fw_name,
			     &handle_to_dev(link)) != 0) {
		printk(KERN_ERR PFX "Cannot find firmware: %s\n",
		       secondary_fw_name);
		return -ENOENT;
