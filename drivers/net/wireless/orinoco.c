...
const static struct fw_info orinoco_fw[] = {
	{ "", "/*(DEBLOBBED)*/", "/*(DEBLOBBED)*/", 0x00390000, 1000 },
	{ "", "/*(DEBLOBBED)*/", "/*(DEBLOBBED)*/", 0, 1024 },
	{ "/*(DEBLOBBED)*/", "/*(DEBLOBBED)*/", "", 0x00003100, 512 }
};
		   ...
		   const struct fw_info *fw)
{
	...
	if (reject_firmware(&fw_entry, fw->pri_fw,
			     priv->dev) != 0) {
		printk(KERN_ERR "%s: Cannot find firmware: %s\n",
		       dev->name, fw->pri_fw);
		return -ENOENT;
	...
	if (reject_firmware(&fw_entry, fw->sta_fw,
			     priv->dev) != 0) {
		printk(KERN_ERR "%s: Cannot find firmware: %s\n",
		       dev->name, fw->sta_fw);
		return -ENOENT;
