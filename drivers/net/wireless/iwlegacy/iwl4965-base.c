...
static int __must_check iwl4965_reject_firmware(struct iwl_priv *priv, bool first)
{
	...
	sprintf(priv->firmware_name, "/*(DEBLOBBED)*/");

	IWL_DEBUG_INFO(priv, "attempting to load firmware '%s'\n",
		       priv->firmware_name);

	return reject_firmware_nowait(THIS_MODULE, 1, priv->firmware_name,
...
iwl4965_ucode_callback(const struct firmware *ucode_raw, void *context)
{
 ...
 try_again:
	/* try next, if any */
	if (iwl4965_reject_firmware(priv, false))
		goto out_unbind;
	release_firmware(ucode_raw);
	return;
...
iwl4965_pci_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	err = iwl4965_reject_firmware(priv, true);
	if (err)
