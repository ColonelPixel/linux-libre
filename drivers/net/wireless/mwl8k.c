				...
				const char *fname, struct firmware **fw)
{
	...
	return reject_firmware((const struct firmware **)fw,
						fname, &priv->pdev->dev);
}

static int mwl8k_reject_firmware(struct mwl8k_priv *priv, u32 part_num)
{
	u8 filename[64];
	int rc;
				 ...
				 const struct pci_device_id *id)
{
	...
	rc = mwl8k_reject_firmware(priv, (u32)id->driver_data);
	if (rc) {
