...
static const struct firmware *ip2_reject_firmware(void)
{
	...
	if (reject_firmware(&fw, "/*(DEBLOBBED)*/", &pdev->dev)) {
		printk(KERN_ERR "Failed to load firmware '/*(DEBLOBBED)*/'\n");
		fw = NULL;
	}
