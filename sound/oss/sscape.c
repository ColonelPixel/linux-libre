...
{	
	...
	unsigned i;
	static	char code_file_name[23] = "/*(DEBLOBBED)*/";
	...
	if (sscape_pnp_upload_file(devc, "/*(DEBLOBBED)*/") == 0 ) {
		printk(KERN_ERR "sscape: faild to upload file /*(DEBLOBBED)*/\n");
	...
	if ( (i & 0x0F) >  7 ) {
		printk(KERN_ERR "sscape: /*(DEBLOBBED)*/ faild\n");
		sscape_pnp_free_dma(devc);
		return;
	}
